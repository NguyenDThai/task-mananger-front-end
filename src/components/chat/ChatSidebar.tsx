...
import { setChats, setMembers } from '../../redux/slides/chat/chatSlide';
  ...
  ({
    ...
    useEffect(() => {
      const fetchInitialData = async () => {
        if (!currentUser?.id || hasInitial.current || chats?.length > 0) return;

        try {
          hasInitial.current = true;
          const [{ data: chatsData }, { data: membersData }] =
            await Promise.all([chat.getChats(), chat.getMembers()]);
      ...
      fetchInitialData();
    }, [currentUser?.id, chats?.length, dispatch]);
            ...
            <div className="divide-y divide-gray-100">
              {filteredChats.map((chat) => (
                <ChatItem
                  ...
                  unreadCount={
                    currentUser?.id ? (chat.new[currentUser.id] ?? 0) : 0
                  }
                />
              ))}
            </div>
